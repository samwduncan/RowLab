...
must_haves:
  ...
  key_links:
    ...
    - from: "src/v2/hooks/useWorkouts.ts"
      to: "/api/v1/training-plans/:id/workouts"
      via: "TanStack Query fetch"
...
@.planning/PROJECT.md
@.planning/ROADMAP.md
@.planning/phases/10-training-plans-ncaa-compliance/10-00-SUMMARY.md
@.planning/phases/10-training-plans-ncaa-compliance/10-01-SUMMARY.md
@src/v2/hooks/useSeatRaceSessions.ts (pattern reference)
@src/v2/types/training.ts
@src/v2/utils/api.ts
@server/routes/trainingPlans.js (backend routes - use these endpoint paths)
</context>
...
**IMPORTANT:** Use the existing backend API paths from server/routes/trainingPlans.js:
- Base path: `/api/v1/training-plans` (NOT `/api/v1/training/plans`)
- Single plan: `/api/v1/training-plans/:id`
- Duplicate: `/api/v1/training-plans/:id/duplicate`
...
Run `npx tsc --noEmit src/v2/hooks/useTrainingPlans.ts` - should compile without errors.
  ...
  <done>
Hook file exports useTrainingPlans, useTrainingPlan, useCreatePlan, useUpdatePlan, useDeletePlan, useDuplicatePlan using correct backend routes.
  </done>
</task>
...
**IMPORTANT:** Use the existing backend API paths from server/routes/trainingPlans.js:
- Workouts are nested under plans: `/api/v1/training-plans/:planId/workouts`
- Update/delete: `/api/v1/training-plans/:planId/workouts/:workoutId`
- For fetching workouts across plans (calendar), use query params on the workouts endpoint
...
async function fetchWorkouts(options: WorkoutListOptions = {}): Promise<PlannedWorkout[]> {
  ...
  // For calendar view (all workouts across plans), need to fetch plans then aggregate
  // Or use a dedicated endpoint if available
  const params = new URLSearchParams();
  if (options.startDate) params.append('startDate', formatISO(options.startDate));
  if (options.endDate) params.append('endDate', formatISO(options.endDate));

  // Note: Backend may need a /api/v1/training-plans/workouts endpoint for cross-plan queries
  // For now, fetch all plans and aggregate their workouts
  const plansResponse = await api.get<TrainingApiResponse<{ plans: { id: string; workouts: PlannedWorkout[] }[] }>>(
    '/api/v1/training-plans'
  );
...
async function updateWorkout(data: Partial<WorkoutFormData> & { id: string; planId: string }): Promise<PlannedWorkout> {
...
async function rescheduleWorkout(data: { id: string; planId: string; scheduledDate: Date }): Promise<PlannedWorkout> {
  const response = await api.put<TrainingApiResponse<{ workout: PlannedWorkout }>>(
    `/api/v1/training-plans/${data.planId}/workouts/${data.id}`,
...
export function useUpdateWorkout() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (updatedWorkout) => {
      queryClient.invalidateQueries({ queryKey: ['workouts'] });
      queryClient.invalidateQueries({ queryKey: ['calendarEvents'] });
      queryClient.invalidateQueries({ queryKey: ['workout', updatedWorkout.planId, updatedWorkout.id] });
    },
  });
...
Run `npx tsc --noEmit src/v2/hooks/useWorkouts.ts` - should compile without errors.
  ...
  <done>
Hook file exports useWorkouts, useCalendarEvents, useWorkout, useCreateWorkout, useUpdateWorkout, useDeleteWorkout, useRescheduleWorkout using correct backend routes.
  </done>
</task>
...
**IMPORTANT:** Use the backend API paths created in 10-00 (server/routes/trainingPlans.js):
- Weekly compliance: `/api/v1/training-plans/compliance/weekly`
- Compliance report: `/api/v1/training-plans/compliance/report`
- Training load: `/api/v1/training-plans/load`
- Attendance link (ATT-04): `/api/v1/training-plans/attendance-link`
...
Run `npx tsc --noEmit src/v2/hooks/useNcaaCompliance.ts` - should compile without errors.
  ...
  <done>
Hook file exports useNcaaWeeklyHours, useNcaaComplianceReport, useTrainingLoad, useAttendanceTrainingLink (ATT-04) using correct backend routes.
  </done>
</task>
...
6. All endpoints match the backend routes in server/routes/trainingPlans.js
</verification>

<success_criteria>
- useTrainingPlans.ts exports plan CRUD hooks using /api/v1/training-plans
- useWorkouts.ts exports workout CRUD hooks using /api/v1/training-plans/:id/workouts
- useNcaaCompliance.ts exports compliance query hooks using /api/v1/training-plans/compliance/*
- useNcaaCompliance.ts exports useAttendanceTrainingLink (ATT-04)
