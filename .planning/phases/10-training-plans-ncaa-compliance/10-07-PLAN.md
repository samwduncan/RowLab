...
must_haves:
  ...
  key_links:
    ...
    - from: "src/v2/hooks/useAssignments.ts"
      to: "/api/v1/training-plans/:id/assign"
      via: "API fetch"
      pattern: "training-plans.*assign"
---
...
- Assign plan: POST `/api/v1/training-plans/:id/assign`
- Delete assignment: DELETE `/api/v1/training-plans/:id/assignments/:assignmentId`
- Mark complete: POST `/api/v1/training-plans/:id/workouts/:workoutId/complete`
- Get athlete load: GET `/api/v1/training-plans/athlete/:athleteId/load`
...
File exists and compiles without TypeScript errors.
  ...
  <done>
useAssignments.ts exports useAssignments, useAthleteAssignments, useCreateAssignment, useDeleteAssignment, useMarkWorkoutComplete using correct backend routes.
  </done>
</task>
...
<verification>
1. useAssignments hook fetches assignments correctly using /api/v1/training-plans endpoints
2. AssignmentManager allows multi-athlete selection
3. AthleteWorkoutView displays workouts grouped by day
4. Mark complete button updates workout status via /api/v1/training-plans/:id/workouts/:workoutId/complete
...
- All API calls use correct backend routes from server/routes/trainingPlans.js
</success_criteria>
