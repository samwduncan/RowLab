...
4. rescheduleWorkout call includes { id, planId, scheduledDate }
  </verify>
  <done>
DragDropCalendar supports drag-to-reschedule with optimistic updates, dragging indicator, proper styling, and correctly passes planId from event.resource to rescheduleWorkout.
  </done>
</task>
...
8. rescheduleWorkout called with { id, planId, scheduledDate } where planId comes from event.resource.planId
</verification>
...
- planId correctly extracted from event.resource and passed to rescheduleWorkout
</success_criteria>
