...
must_haves:
  truths:
    - "CSV import follows Phase 7 column mapping pattern"
    - "Coach can preview data before import"
    - "Validation shows errors for invalid rows"
    - "Bulk import creates results for selected race"
  artifacts:
    - path: "src/v2/components/regatta/ResultsCSVImport.tsx"
      provides: "Multi-step CSV import wizard for race results"
      exports: ["ResultsCSVImport"]
  key_links:
    - from: "src/v2/components/regatta/ResultsCSVImport.tsx"
      to: "src/v2/hooks/useRaces.ts"
      via: "imports useBatchAddResults hook"
      pattern: "import.*useBatchAddResults.*from.*hooks"
---

<objective>
Create CSV import component for bulk race results entry following Phase 7 patterns.

Purpose: Enable coaches to import results from large regattas via CSV with flexible column mapping.
Output: ResultsCSVImport component with upload, column mapping, preview, and import steps.
</objective>
...
@src/v2/components/erg/ErgCSVImportModal.tsx (pattern reference)
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create ResultsCSVImport wizard component</name>
  <files>src/v2/components/regatta/ResultsCSVImport.tsx</files>
  <action>
Create src/v2/components/regatta/ResultsCSVImport.tsx following Phase 7 ErgCSVImportModal pattern:
...
import { Upload, FileText, AlertCircle, Check, ChevronLeft, ChevronRight, X } from 'lucide-react';
...
```
  ...
  <verify>
Upload step accepts CSV. Map step auto-detects columns. Preview shows valid/invalid counts. Import creates results.
  </verify>
  <done>
ResultsCSVImport provides complete 4-step workflow: upload → map columns → preview with validation → import with results.
  </done>
</task>

<task type="auto">
  <name>Task 2: Update barrel exports</name>
  <files>src/v2/components/regatta/index.ts</files>
  <action>
Add ResultsCSVImport to barrel exports:
...
```
  ...
  <verify>
Import from barrel works: `import { ResultsCSVImport } from '@/v2/components/regatta'`
  </verify>
  <done>
All regatta components exported from barrel.
  </done>
</task>
...
<verification>
1. `npx tsc --noEmit` on ResultsCSVImport
2. Upload CSV with various formats (with/without header)
3. Auto-detect finds common column names
4. Preview shows validation errors for bad rows
5. Import creates results and invalidates queries
</verification>

<success_criteria>
- CSV upload works with drag-drop and click
- Column mapping auto-detects common headers
- Preview shows valid/invalid row counts
- Invalid rows show specific error messages
- Import creates results via batch endpoint
</success_criteria>

<output>
After completion, create `.planning/phases/11-racing-regattas/11-05-SUMMARY.md`
</output>
