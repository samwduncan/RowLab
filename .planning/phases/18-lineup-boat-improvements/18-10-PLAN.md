...
must_haves:
  truths:
    - "Historical browser displays searchable/filterable lineup list"
    - "Filters include athletes, boat class, date range"
    - "Results show metadata (athlete count, boat classes)"
  artifacts:
    - path: "src/v2/features/lineup/components/HistoricalLineupBrowser.tsx"
      provides: "Searchable lineup history with filters"
      min_lines: 200
  key_links:
    - from: "src/v2/features/lineup/components/HistoricalLineupBrowser.tsx"
      to: "src/v2/hooks/useLineupSearch.ts"
      via: "useLineupSearch hook"
      pattern: "useLineupSearch"
---

<objective>
Create historical lineup browser component.

Purpose: Enable coaches to search and filter past lineups for pattern analysis and reuse.
Output: UI component with multi-criteria search, filters, and results display.
</objective>
...
<task type="auto">
  <name>Task 1: Create HistoricalLineupBrowser component</name>
  <files>src/v2/features/lineup/components/HistoricalLineupBrowser.tsx</files>
  <action>
Create `src/v2/features/lineup/components/HistoricalLineupBrowser.tsx`:
...
export function HistoricalLineupBrowser({
  ...
  const handleCompare = useCallback(() => {
    if (onCompare && compareIds.length === 2) {
...
```
  ...
  <verify>Run `npx tsc --noEmit src/v2/features/lineup/components/HistoricalLineupBrowser.tsx` - should compile without errors.</verify>
  <done>HistoricalLineupBrowser component created with search, filters, and comparison selection.</done>
</task>

<task type="auto">
  <name>Task 2: Export component from index</name>
  <files>src/v2/features/lineup/components/index.ts</files>
  <action>
Update `src/v2/features/lineup/components/index.ts` to include new export:

```typescript
export { HistoricalLineupBrowser } from './HistoricalLineupBrowser';
```
  </action>
  <verify>Run `npx tsc --noEmit src/v2/features/lineup/components/index.ts` - should compile without errors.</verify>
  <done>HistoricalLineupBrowser exported from index.</done>
</task>

</tasks>

<verification>
- [ ] `npx tsc --noEmit src/v2/features/lineup/components/HistoricalLineupBrowser.tsx` passes
- [ ] Component exported from index.ts
- [ ] Search functionality works
- [ ] Filters include athletes, boat class, date range
- [ ] Comparison selection supports two lineups
</verification>

<success_criteria>
1. Search by lineup name works
2. Filter by boat class, date range works
3. Athlete filter with "at least N" slider works
4. Results show metadata (athlete count, boats, matched count)
5. Compare selection allows selecting two lineups
</success_criteria>

<output>
After completion, create `.planning/phases/18-lineup-boat-improvements/18-10-SUMMARY.md`
</output>
