...
interface UseRegattaKeyboardConfig {
  /** Called when N is pressed — create new regatta */
  onNewRegatta?: () => void;
  /** Called when R is pressed — refresh data */
  onRefresh?: () => void;
  /** Called when Escape is pressed — close open panels/modals */
  onEscape?: () => void;
  /** Called when E is pressed — export rankings (only on rankings page) */
  onExport?: () => void;
  /** Master toggle — disables all keyboard shortcuts when false */
  enabled?: boolean;
}

interface UseRegattaKeyboardReturn {
...
/**
 * Keyboard shortcut handler for regattas and rankings pages.
 *
 * Shortcuts:
 * - N: Create new regatta (opens RegattaForm) — skip if in input/textarea/dialog
 * - R: Refresh data (invalidate regatta/ranking queries)
 * - ?: Show help overlay (keyboard shortcut reference card)
 * - Escape: Close any open panel, modal, or form
 * - E: Export rankings (only on rankings page, if onExport provided)
 *
 * All shortcuts are context-aware and skip when the user is
 * typing in input fields, dialogs, or the command palette.
 */
export function useRegattaKeyboard(
  config: UseRegattaKeyboardConfig = {}
): UseRegattaKeyboardReturn {
  const { onNewRegatta, onRefresh, onEscape, onExport, enabled = true } = config;
  ...
  const handleKeyDown = useCallback(
      ...
      switch (e.key) {
        ...
        case 'R': {
          ...
          if (onRefresh) {
            ...
            // Default: invalidate regatta and ranking queries
    ...
    [enabled, showHelp, onNewRegatta, onRefresh, onEscape, onExport, queryClient]
  );
...
export interface RegattaShortcut {
  key: string;
  description: string;
  available: boolean;
}
 ...
 */
export function getRegattaShortcuts(config: {
  hasNewRegatta?: boolean;
  hasRefresh?: boolean;
  hasEscape?: boolean;
  hasExport?: boolean;
}): RegattaShortcut[] {
  return [
    { key: 'N', description: 'Create new regatta', available: config.hasNewRegatta ?? false },
    { key: 'R', description: 'Refresh data', available: config.hasRefresh ?? true },
    { key: 'E', description: 'Export rankings', available: config.hasExport ?? false },
