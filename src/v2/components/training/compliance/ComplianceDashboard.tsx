...
export function ComplianceDashboard({
  ...
  const summaryStats = useMemo(() => {
    ...
    const athletesNear = complianceEntries.filter((e: NCAAAuditEntry) => e.isNearLimit && !e.isOverLimit).length;
    const totalAthletes = complianceEntries.length;
    const avgHours = totalAthletes > 0
      ? complianceEntries.reduce((sum: number, e: NCAAAuditEntry) => sum + e.totalHours, 0) / totalAthletes
      : 0;
